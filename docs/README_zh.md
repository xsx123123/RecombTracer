...
- **`recombtracer.recombiner`** — PBWT 风格单倍型涂染，基于最大匹配片段将每个子代的 SNP 分配到最可能的亲本来源。
- **`recombtracer.hmm`** — HMM 细化，将嘈杂的 PBWT 分配平滑为连续的祖先区块，并以置信度评分调用重组断点。
...
## VCF 转换（`recombtracer.vcf`）

如果你的数据从 VCF 文件开始（例如变异检测和过滤后），请使用 `recombtracer.vcf` 提取单倍型并将其转换为 `MagicRecombiner` 使用的矩阵格式。
